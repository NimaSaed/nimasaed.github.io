...
If all five checks pass, the request proceeds.
If any check fails, the access token cookie is cleared, and the client is redirected to re-authenticate. This prevents silent session continuation after a detected tampering attempt.
